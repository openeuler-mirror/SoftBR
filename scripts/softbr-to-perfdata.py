...
parser.add_argument("-b", "--buildid", help="build-id list file", type=str, required=True)
parser.add_argument("-o", "--output", help="output file name, default: perf.data", type=str, default="perf.data")
...
def parse_buildid_list(buildid_file):
    buildid_items = []
    with open(buildid_file, 'r') as f:
        for line in f:
            buildid, path = line.strip().split(' ')
            bytes_list = []
            for i in range(0, len(buildid), 2):
                bytes_list.append(int(buildid[i:i+2], 16))
            buildid_items.append((bytes_list, path))
    return buildid_items
...
def build_buildid(buildid_items):
    data = bytes(0)
    for buildid, filename in buildid_items:
        data += struct.pack('<I', 0)
        data += struct.pack('<H', 0x8002) #misc
        filename_len = int((len(filename) + 8 + 4) / 8) * 8
        data += struct.pack('<H', 8 + 4 + 24 + filename_len) # size
        data += struct.pack('<I', g_pid)
        data += struct.pack('20s', bytes(buildid))
        data += struct.pack('<B', len(buildid))
        data += struct.pack('<B', 0)
        data += struct.pack('<H', 0)
        data += struct.pack(f'{filename_len}s', filename.encode('utf-8'))
    return data

def build_perf_data(input_file, buildid_file):
    brstack_items, mmap_items, mmap2_items = parse_perf_script(input_file)
    buildid_items = parse_buildid_list(buildid_file)
    ...
    buildid_data = build_buildid(buildid_items)
    ...
    header += struct.pack('<Q', 0x4)            # flags
    ...
    data += mmap2_data
    data += brstack_data
    data += struct.pack('<Q', len(header) +  len(attr_data) + 16 + len(mmap_data) + len(mmap2_data) + len(brstack_data) + 8 + 8)
    data += struct.pack('<Q', len(buildid_data))
    data += buildid_data
    return data
...
def main():
    data = build_perf_data(f"{args.input}", f"{args.buildid}")
    write_file(data)
